...
触发关键词：Method-A1、OpenDSS、稠密监督、候选响应特征、TCN、GNN、反事实验证
检索顺序：4
...
本设计将 Method-A1 的 3.1 端到端框架落地为可运行的 IEEE13 初始实现。系统使用 OpenDSS 对每个候选故障母线执行离线干预，训练一个条件响应特征预测器，并以预测波形与观测波形的残差完成定位和无故障检测。
...
- 离线 OpenDSS 响应特征库，不在训练步骤中调用 COM；
- 单模型、单推理方式，不接入 Method-C 的 MMoE、定位头、响应特征解码器和训练器。
...
X_obs ──> TCN 时序编码器 ──> 节点时序表示
                      ...
                      节点表示 h 与全局表示 g
                                  │
             (h, g, candidate_idx) ──> A1 候选响应特征解码器
                                  │
                     预测响应特征 Ŝ[B,C,N,T,6]
                                  │
                 观测 mask 下计算候选残差 r(k)
...
Method-C 的 `dataset_builder.py` 只作为数据字段和场景编码参考，A1 重写数据集构建器，以便保存每个样本的全候选反事实响应特征。
...
- `signature_bank.npy`：稠密候选响应特征，形状 `[M,N+1,N,T,6]`；
...
为支持后续真实样本混合训练，数据接口额外允许 `signature_valid` 和 `real_sample` 字段；首轮 OpenDSS 数据中所有故障候选响应特征均有效。
...
对每个候选索引 `k`，取候选嵌入 `e_k` 与候选母线节点表示 `h[:,k,:]`；对 `NO_FAULT` 使用固定、不参与训练的 `e_0`。解码器将以下向量拼接后通过直接 MLP 生成完整响应特征：
...
当 `signature_valid[k]=0` 时，不计算该候选的绝对响应特征 MSE；若存在真实位置，则计算真实候选回归和负候选排序：
...
S0 至少报告响应特征 MSE、真实候选残差、Top-1/Top-K、平均真实排名、检测准确率、故障召回率、F1、无故障残差差 `d` 和残差间隔。S1 拓扑错误和 S2 部分观测作为后续独立实验，不纳入本轮数据生成、训练、报告与验收。
...
- `src/data_generation/dataset_builder.py`：A1 离线数据与全候选响应特征库；
- `src/model/temporal.py`：迁移的 TCN；
- `src/model/gnn.py`：去掉边可信度后的普通拓扑 GNN；
- `src/model/signature_predictor.py`：候选条件响应特征解码器；
...
1. 不依赖 COM 的单元测试全部通过；
2. Fake simulator 端到端 smoke 能生成离线响应特征库、训练一个 epoch、完成残差定位并保存 JSON 报告；
...
- 采用完整波形响应特征而非静态相量，以保留 TCN 提取的时序信息并直接支持 3.1 残差定义；
- 采用普通拓扑 GNN 而非边可信度 GNN，避免引入用户明确不需要的边可信度任务；
- 首轮选择 IEEE13、S0 和全枚举，优先验证反事实稠密监督链路；S1/S2、更大馈线和困难候选采样留作后续实验；
- 使用离线响应特征库隔离 OpenDSS 的高昂 COM 调用，确保训练可复现且 GPU 循环不被仿真器阻塞；
