<!-- 摘要：本文档为项目 docs 目录的统一索引，规定文档检索顺序、适用范围、Method-A1 主题文档、诊断表示空间逻辑链、方法前证据审计、实验—命题对应关系及更新约束。 -->
...
- `docs/project/Method-A1 新计划：响应特征空间差异来源与诊断关系的统一目标.md`：给出从原始物理响应特征空间评估到诊断表示空间构建的完整逻辑链，明确故障信息强化、无关干扰控制、显式因素识别与隐式不变表示选择、诊断关系优化及经验证物理约束保持要求。

- `docs/project/Method-A1-具体方法确认前证据审计与验证计划.md`：审计既有文档与实验产物，区分已验证、初步验证和未验证命题，明确每项待验证命题对应的实验编号、评价证据和结论边界，并重点规定 S2 部分观测与 S4 高阻故障的补充验证方案。
