<!-- 摘要：本文档为项目 docs 目录的统一索引，规定文档检索顺序、适用范围、Method-A1 主题文档、信息充分性、模板覆盖归因、配对因素来源、诊断表示空间决策链、E0–E7 可执行实验规程及更新约束。 -->
...
- `docs/project/Method-A1-E0-E7可执行实验流程与决策规程.md`：将 H0–H7 细化为条件式执行主链和逐实验规程，记录 E0 正式结果，并以 E0-COV 模板覆盖消融、E1-A 跨工况排序、E1-B 故障—工况配对和 E1-C 局部错排结构规定进入 E2/E4/E5 的条件；不预先指定表示学习方法。

- `docs/project/method-a1-status.md`：Method-A1 当前阶段、E0 信息充分性正式证据、既有实验基线、契约缺口、局限、阻塞项和下一阶段决策；不承载工作日志或逐次运行记录。
...
- `docs/project/Method-A1-S1-S4响应特征库与物理邻近性实验设计.md`：规定 S1–S4 Oracle 实验的响应特征库持久化、复用校验和物理邻近节点响应特征相似性分析。
