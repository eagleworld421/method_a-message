<!-- 摘要：本文档为项目 docs 目录的统一索引，规定文档检索顺序、适用范围、Method-A1 主题文档、信息充分性与噪声边界、诊断表示空间决策链、E0–E7 可执行实验规程、独立瓶颈归因、实验—命题对应关系及更新约束。 -->
...
- `docs/project/Method-A1-E0-E7可执行实验流程与决策规程.md`：将 H0–H7 细化为 E0–E7 的条件式执行主链和逐实验规程，规定数据与配对、分组防泄漏、统计判据、输出契约和代码复用边界，并记录 E0 已冻结协议、正式结果及进入 E1 的限定条件；不预先指定表示学习方法。
